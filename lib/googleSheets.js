...
export async function getSheetBreakdown(sheetId, sheetName) {
  if (!sheetId) return [];
  const cacheBuster = Math.floor(Date.now() / 10000);
  const url = `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?tqx=out:json&sheet=${sheetName}&cb=${cacheBuster}`;
  ...
  try {
    ...
    return data.table.rows.map(row => {
      const name = row.c && row.c[0] ? String(row.c[0].v || '').trim() : '';
      const count = row.c && row.c[1] ? Number(row.c[1].v) || 0 : 0;
      return { name, count };
    }).filter(item => item.name !== '');
  } catch (error) {
    console.error('Error fetching breakdown sheet:', error);
    return [];
  }
}
