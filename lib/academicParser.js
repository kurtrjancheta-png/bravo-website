...
export async function getAcademicDeficiencies() {
  ...
  // Fetch Roster to build surname -> class mapping for data validation
  const rosterSheetId = process.env.ROSTER_SHEET_ID || '1HoTX11Y0Ojx_Ow99J93mRxNAOBpcGods55bpggYxAdk';
  const rosterRows = await fetchSheetCSV(rosterSheetId, 'ROSTER');
  const rosterMap = {};
  if (rosterRows && rosterRows.length > 0) {
    const headers = rosterRows[0].map(h => h.trim().toUpperCase());
    const classColIdx = headers.indexOf('CLASS');
    const surnameColIdx = headers.indexOf('SURNAME');
    if (classColIdx !== -1 && surnameColIdx !== -1) {
      for (let i = 1; i < rosterRows.length; i++) {
        const row = rosterRows[i];
        const cl = row[classColIdx] ? String(row[classColIdx]).trim().toUpperCase() : '';
        const surname = row[surnameColIdx] ? String(row[surnameColIdx]).trim().toUpperCase() : '';
        if (surname && cl) {
          rosterMap[surname] = cl;
        }
      }
    }
  }
  ...
  for (const tab of tabs) {
    ...
    for (const row of dataRows) {
      ...
      // Class validation using company roster
      if (Object.keys(rosterMap).length > 0) {
        const cleanName = name.trim().toUpperCase();
        let rosterClass = '';
        if (rosterMap[cleanName]) {
          rosterClass = rosterMap[cleanName];
        } else {
          // Check prefix match (e.g. "ASUNCION AJL" starts with "ASUNCION")
          for (const [surname, cl] of Object.entries(rosterMap)) {
        ...
        // If the cadet is found in the roster, but their class is different from this tab's class, filter them out!
        if (rosterClass && rosterClass !== className) {
          console.log(`[Academic Parser] Filtered out cadet ${name} from class tab ${className} because their roster class is ${rosterClass}`);
          continue;
        }
      }
