...
export function parsePFTData(rows, genderMap = {}) {
  ...
  rows.forEach((row) => {
    ...
    if (val.includes('PASSED') || val === 'P') {
      ...
      if (gender === 'F') {
        updateTop('pushupsF', pushupsRaw);
        updateTop('situpsF', situpsRaw);
        updateTop('flexarm', pullupsRaw);
        updateTop('runF', runRaw, true);
      } else {
